...
resource "oci_core_instance" "redis_replica" {
  ...
  display_name        = "${var.redis_prefix}${count.index + ((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_replica_count:var.redis_rediscluster_slave_count)}"
  ...
  create_vnic_details {
    ...
    hostname_label   = "${var.redis_prefix}${count.index + ((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_replica_count:var.redis_rediscluster_slave_count)}"
  }
